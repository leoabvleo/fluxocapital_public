#!/bin/bash
# Script: import-db-oracle2davinci.sh
# Utilidade: Baixar o último backup do servidor Oracle e importar no banco local (Davinci)
...
echo "==============================================="
echo "📥 IMPORTANDO BANCO (PRODUÇÃO -> DAVINCI-STI)"
echo "==============================================="

# 1. Identificar o backup mais recente no servidor Oracle via SSH
echo "🔍 Buscando backup mais recente no servidor Oracle..."
LATEST_BACKUP=$(ssh -o KexAlgorithms=+diffie-hellman-group14-sha1 oracle "ls -t /home/ubuntu/db_fluxocapital_prod_*.sql.gz 2>/dev/null | head -n 1")
...
echo "🚚 Baixando arquivo para o Davinci-STI..."
if scp -o KexAlgorithms=+diffie-hellman-group14-sha1 ubuntu@144.22.239.218:"$LATEST_BACKUP" .; then
